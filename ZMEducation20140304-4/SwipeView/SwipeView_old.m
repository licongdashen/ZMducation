...
- (void)setFrameForView:(UIView *)view atIndex:(NSInteger)index
{
    ...
    {
        CGPoint _cp = CGPointMake(([self offsetForItemAtIndex:index] + 0.5f) * _itemSize.width + _scrollView.contentOffset.x, _scrollView.frame.size.height/2.0f);
        NSLog(@"self offsetForItemAtIndex:%f",[self offsetForItemAtIndex:index]);
        
        CGPoint cp = CGPointMake(1024.0f/2.0f, 768.0f/2.0f);
        view.center = _cp;
