...
- (void)viewDidLoad {
    ...
    for (NSDictionary *dic in self.unitArray) {
        if ([dic[@"articleType"] intValue] == 98) {
            ...
            UIImageView *contentImagv = [[UIImageView alloc]initWithFrame:CGRectMake(100,titleImagv.frame.origin.y + titleImagv.frame.size.height + 20 , self.view.frame.size.width - 200, 500)];
            contentImagv.image = [UIImage imageNamed:@"Article_Item_12"];
            contentImagv.userInteractionEnabled = YES;
            [self.scro addSubview:contentImagv];
            
            self.TV_Bk2 = [[UIExpandingTextView alloc]initWithFrame:CGRectMake(60, 60 , contentImagv.frame.size.width - 120, contentImagv.frame.size.height - 120)];
            self.TV_Bk2.font = [UIFont systemFontOfSize:18];
            self.TV_Bk2.backgroundColor = [UIColor clearColor];
            [contentImagv addSubview:self.TV_Bk2];
        ...
        if ([dic[@"articleType"] intValue] == 99) {
            ...
            UIImage* article_Category_Image = [UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"Article_Category_bg" ofType:@"png"]];
            UIImageView* article_Category_View = [[UIImageView alloc] initWithFrame:CGRectMake(self.view.frame.size.width*([self.unitArray count] -1) + 291, 15, 421, 43)];
            [article_Category_View setImage:article_Category_Image];
            [self.scro addSubview:article_Category_View];
            [article_Category_View release];
            ...
            [browse_But setFrame:CGRectMake(self.view.frame.size.width*([self.unitArray count] -1) + 317 + 100, 252, 171, 40)];
            [browse_But setTag:99];
            ...
            [photo_But setFrame:CGRectMake(self.view.frame.size.width*([self.unitArray count] -1) + 250 + 100, 247, 49, 50)];
            [photo_But setTag:99];
...
-(void)submitWorkClick
{
    ...
    [requestDict setValue:[userDict valueForKey:@"currentGradeId"] forKey:@"gradeId"];
    [requestDict setValue:[userDict valueForKey:@"currentModuleId"] forKey:@"moduleId"];
    [requestDict setValue:[self.onedic valueForKey:@"designId"] forKey:@"unitId"];
    [requestDict setValue:[userDict valueForKey:@"userId"] forKey:@"userId"];
    [requestDict setValue:@"98" forKey:@"articleType"];
    [requestDict setValue:self.TV_Bk1.text forKey:@"articleTitle"];
    [requestDict setValue:self.TV_Bk2.text forKey:@"articleDraft"];
