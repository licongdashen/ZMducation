...
- (void)setUseThumbnailView:(BOOL)useThumbnailView
{
    ...
    if( self.navigationController ) {
        if (_useThumbnailView) {
            UIBarButtonItem *left_btn = [[UIBarButtonItem alloc] initWithTitle:@"完成"
                                                                          style:UIBarButtonItemStylePlain
                                                                         target:self
                                                                         action:@selector(doneTouch:)];
            [self.navigationItem setLeftBarButtonItem:left_btn animated:YES];
            [left_btn release];
            
            UIBarButtonItem *right_btn = [[[UIBarButtonItem alloc] initWithTitle:@"全部"
                                                                           style:UIBarButtonItemStylePlain
                                                                          target:self
                                                                          action:@selector(handleSeeAllTouch:)] autorelease];
            [self.navigationItem setRightBarButtonItem:right_btn animated:YES];
        }
        else {
...
-(IBAction)collectButClick:(id)sender{
    [_photoSource collectButtonDidClick:self];
}

-(IBAction)shareClick:(id)sender{
    [_photoSource shareButtonDidClick:self];
}
		...
		if([_photoSource respondsToSelector:@selector(photoGallery:captionForPhotoAtIndex:)])
		{
			...
			if([caption length] > 0 )
			{
                ...
                if (_flag) {
                    ...
                    [_toolbar addSubview:collectBut];
                    
                    UIButton* share_But = [UIButton buttonWithType:UIButtonTypeCustom];
                    [share_But setFrame:CGRectMake(100, 0, 171, 40)];
                    [share_But setBackgroundImage:[UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"Unit_Common_Button" ofType:@"png"]] forState:UIControlStateNormal];
                    [share_But setBackgroundImage:[UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"Unit_Common_Button" ofType:@"png"]] forState:UIControlStateHighlighted];
                    [share_But setTitle:@"分享" forState:UIControlStateNormal];
                    [share_But addTarget:self
                                  action:@selector(shareClick:)
                        forControlEvents:UIControlEventTouchUpInside];
                    [_toolbar addSubview:share_But];
                }
